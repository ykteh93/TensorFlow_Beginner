...
weights = {'hidden_1': tf.Variable(tf.random_normal([number_input, features_1])),
           'out'     : tf.Variable(tf.zeros([features_1, number_classes]))}

biases  = {'hidden_1': tf.Variable(tf.random_normal([features_1])),
          'out'      : tf.Variable(tf.zeros([number_classes]))}

# Hidden Layer with Relu  -> Predict the label
layer_1   = tf.nn.relu(tf.matmul(input_x, weights['hidden_1']) + biases['hidden_1'])
predict_y = tf.matmul(layer_1, weights['out']) + biases['out']
...
with tf.Session() as sess:
    ...
    sys.stdout = open('Parameter_P1_b (All Weights & Biases).txt', 'w')
