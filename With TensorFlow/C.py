...
weights = {'hidden_1': tf.Variable(tf.random_normal([number_input, features_1])),
           'hidden_2': tf.Variable(tf.random_normal([features_1, features_2])),
           'out': tf.Variable(tf.random_normal([features_2, number_classes]))}

biases = {'hidden_1': tf.Variable(tf.random_normal([features_1])),
          'hidden_2': tf.Variable(tf.random_normal([features_2])),
          'out': tf.Variable(tf.random_normal([number_classes]))}

# Hidden Layer with Relu  -> Hidden Layer with Relu  -> Predict the label
layer_1 = tf.nn.relu(tf.matmul(input_x, weights['hidden_1']) + biases['hidden_1'])
layer_2 = tf.nn.relu(tf.matmul(layer_1, weights['hidden_2']) + biases['hidden_2'])
predict_y = tf.matmul(layer_2, weights['out']) + biases['out']
...
with tf.Session() as sess:
    ...
    sys.stdout = open('Parameter_P1_c (All Weights & Biases).txt', 'w')
    print('------------------ Weights First Hidden Layer ------------------')
    print(weights['hidden_1'].eval())
    print('\n------------------ Weights Second Hidden Layer ------------------')
    print(weights['hidden_2'].eval())
    print('\n------------------ Weights Final Layer ------------------')
    print(weights['out'].eval())
    print('\n------------------ Biases First Hidden Layer ------------------')
    print(biases['hidden_1'].eval())
    print('\n------------------ Biases Second Hidden Layer ------------------')
    print(biases['hidden_2'].eval())
    print('\n------------------ Biases Final Layer ------------------')
