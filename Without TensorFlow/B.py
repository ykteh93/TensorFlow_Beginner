...
def feed_forward(x,true_y):
    ...
    predict_y = softmax(layer_1.dot(weights['out']) + biases['out'])
    mistake = (np.sum(np.argmax(predict_y, axis=1) != np.argmax(true_y, axis=1))) / len(true_y)
    return layer_1, predict_y, mistake


# Backward propagation
def feed_backward(layer_1, predicted_y, x, true_y):
    delta = (predicted_y - true_y) / len(true_y)
    dW = (layer_1.T).dot(delta)
    dB = np.sum(delta, axis=0, keepdims=True)

    dh1 = np.dot(delta, weights['out'].T)
    dh1[layer_1 <= 0] = 0
    dW_h1 = np.dot(x.T, dh1)
    dB_h1 = np.sum(dh1, axis=0, keepdims=True)

    return dW, dW_h1, dB, dB_h1
...
def gradient_descent(train_x, train_y, test_x, test_y):
    ...
    for epochs in range(batch_size * number_epochs):
        ...
        layer_1, predict_y, _  = feed_forward(minibatch_x, minibatch_y)
        dW, dW_h1, dB, dB_h1 = feed_backward(layer_1, predict_y, minibatch_x, minibatch_y)
        ...
        if epochs % batch_size == 0:
            _, _, error_mini_train = feed_forward(minibatch_x, minibatch_y)
            _, _, error_full_train = feed_forward(train_x, train_y)
            _, _, error_test = feed_forward(test_x, test_y)
    ...
    _, _, error_full_train = feed_forward(train_x, train_y)
    _, predict_test_y, error_test = feed_forward(test_x, test_y)
...
features_1 = 128     # Number of features for the first hidden layer
...
weights = {'hidden_1': np.random.standard_normal((number_input, features_1)),
           'out': np.zeros((features_1, number_classes))}

biases = {'hidden_1': np.random.standard_normal((1, features_1)),
          'out': np.zeros((1, number_classes))}
...
np.savez('./model/weights.npz', out=weights['out'], hidden_1=weights['hidden_1'])
np.savez('./model/biases.npz', out=biases['out'], hidden_1=biases['hidden_1'])

# Store the parameter value of weight and bias into text file
sys.stdout = open('Parameter_P2_c.txt', 'w')
print('------------------ Weights First Hidden Layer ------------------')
print(weights['hidden_1'])
print('\n------------------ Weights Final Layer ------------------')
print(weights['out'])
print('\n------------------ Biases First Hidden Layer ------------------')
print(biases['hidden_1'])
print('\n------------------ Biases Final Layer ------------------')
print(biases['out'])