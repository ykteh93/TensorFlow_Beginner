...
def feed_forward(x,true_y):
    ...
    return layer_1, predict_y, accuracy, loss
...
def gradient_descent(train_x, train_y, test_x, test_y):
    ...
    for epochs in range(number_epochs):
        for e in range(int(mnist.train.num_examples / batch_size)):
            ...
            layer_1, predict_y, _, _  = feed_forward(minibatch_x, minibatch_y)
        ...
        _, _, train_acc, train_loss             = feed_forward(train_x, train_y)
        _, predict_test_y, test_acc, test_loss  = feed_forward(test_x, test_y)
